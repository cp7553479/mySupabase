...
where not exists (
  select 1
  from public.product_media as existing_media
  where existing_media.product_id = product.id
    and existing_media.media_asset_id = media_asset.id
    and existing_media.usage_type = 'gallery'
);
