...
- 每次 Git 提交前，完成对应的实际 Browser 调试、仿真测试和视觉验收；仅在验证通过后提交。


## 项目文档

- `skills/project-initialization/SKILL.md`：项目初始化 Skill。开始新项目、启动需求调研、参考同类网站形成问卷，或在创建/大幅重构 `SPEC.md` 前信息不完整时，先调用此 Skill 了解基础信息。
- `INIT_QUESTIONNAIRE.md`：项目初始化需求采集问卷，用于在创建或大幅重构 `SPEC.md` 前收集品牌、企业、商品、转化、内容、合规和上线基础信息。
